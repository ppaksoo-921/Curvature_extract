...
def extract_curvature(new_cp, cp_cut, where, ii):
    ...
    put_coord = np.zeros((num_curve, 700, 2))
...
with open("C:/Users/owner/Desktop/Curvature/prestack.bin") as f:
    #data = np.fromfile(f, dtype=np.float32).reshape(58096,-1)[120+130*16:120+131*16,:1500]
    data = np.fromfile(f, dtype=np.float32).reshape(58096,-1)[3384:3400,:1500]
...
win_len=120
draw=320
for start in range(draw,draw+1,100):
    ...
    for i in range(array.shape[0]):  # i 가 y값
        for j in range(array.shape[1]):  # j 가 x 값
