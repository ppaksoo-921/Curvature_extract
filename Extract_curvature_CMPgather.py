...
def extract_curvature(new_cp, cp_cut, where, ii):
    ...
    put_coord = np.zeros((num_curve, 700, 2))   # Array storing the coordinates occupied by all curves  
                                                # → If the new dataset includes more regions covered by curves,  
                                                #   the array size must be increased accordingly (e.g., from 700×2 to 2500×2)
    ...
    for ic in range(num_curve):
        ...
        check_long = np.any(put_coord[ic, :, 1] == 12)
    ...
    result_combo_2nd = np.zeros((real_curve_num, 16))
    result_error_2nd = np.zeros(real_curve_num)
    ...
    for ic in range(real_curve_num):
        ...
        # Step 4: Exclude combinations that are too far apart
        valid_combinations = [
            combo for combo in combinations
            if all(abs(combo[i] - combo[i + 1]) <= 20 for i in range(len(combo) - 1))
        ]
        ...
        for combo in valid_combinations:
            ...
            X = np.vstack([x_val ** 2, np.ones(len(x_val))]).T
            adjuseted_y = y_val - p
            a = np.linalg.lstsq(X, adjuseted_y, rcond=None)[0][0]

            errors = 0
            for ix in range(len(x_val)):
                errors = errors + abs(a * ix ** 2 - adjuseted_y[ix])

            if abs(end_curve_point - combo[len(combo)-1]) <= 15 or end_curve_point == 1000:
...
#A function that stacks 1D curvature information obtained at each midpoint to construct a 2D guideline
def make_guideline(array_a, array_p, num):
    heat_map=np.zeros((1500,num))

    for i_cdp in range(num):

        tot_a = array_a[i_cdp]
        tot_p = array_p[i_cdp]

        for i in range(len(tot_p)):
            heat_map[int(tot_p[i]) - 2][i_cdp] = -abs(tot_a[i])
            heat_map[int(tot_p[i])-1][i_cdp] = -abs(tot_a[i])
            heat_map[int(tot_p[i])][i_cdp]=-abs(tot_a[i])
            heat_map[int(tot_p[i])+1][i_cdp] = -abs(tot_a[i])
            heat_map[int(tot_p[i]) +2][i_cdp] = -abs(tot_a[i])
    ...
    heat_map_cut=heat_map[:500,:]

    heat_map.tofile('image.bin')

    plt.imshow(heat_map_cut, aspect="auto", cmap="PuOr",clim=(max_value*0.1, -0.1*max_value))  
    plt.colorbar(label="Value")  
    plt.xlabel("Index")

    plt.savefig(f"C:/Users/owner/Desktop/Result_CMP_gather/guideline.png", dpi=300, bbox_inches="tight")   # Put your output directory here
    plt.show()
    plt.close()  
...
# Select the CMP gather to be processed
with open("./prestack.bin") as f:
    tot_data = np.fromfile(f, dtype=np.float32).reshape(58096,-1)[:,:1500]
...
for i_cdp in range(200,250):  # Perform curvature extraction by iterating over each midpoint
    pos_cdp = 120 + 16 * i_cdp
    data = tot_data[pos_cdp:pos_cdp + 16, :]
    ...
    win_len=200
    draw=50
    for start in range(draw,1500,100):
        cmp_gather = data[:16, start:start + win_len]
        array = cmp_gather.T
        ...
        for ix in range(16):
            ...
            for i in range(cp.shape[0]-1):
                ...
                if cp[i][ix]!=0 and cp[i+1][ix]==0:
                    t_end=i
                    status=1
                if status==1 and t_end-t_start>3:
    ...
    np.save(f"./result_0924/tot_a_{i_cdp}", tot_result_a)
    np.save(f"./result_0924/tot_p_{i_cdp}", tot_result_p)


    all_cdp_a.append(tot_result_a)  # Curvature data obtained from one midpoint is stored in this array  
    all_cdp_p.append(tot_result_p)  # and later used for guideline construction

    '''
    if i_cdp % 100 == 0:
        plt.figure()
        plt.scatter(tot_result_a, tot_result_p)
        plt.xlabel('a coefficient')
        plt.ylabel('p coefficient')
        plt.title(f'Curvature Plot of {i_cdp}')
        plt.grid(True, linestyle='--', linewidth=0.5)
        plt.ylim(500, 100)
        plt.show()
    '''

make_guideline(all_cdp_a, all_cdp_p, 50)
