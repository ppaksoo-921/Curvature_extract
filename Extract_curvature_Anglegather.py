...
SHOW_FIGURE=False
SHOW_heatmap=True
# SAVE_FIGURE: Controls whether to save each angle_gather figure. Recommended: True.
# SHOW_FIGURE: Controls whether to display each angle_gather figure. Generally keep False.
# SHOW_heatmap: Controls whether to save the guideline heatmap. Recommended: True.

folder="base"
# folder: Selects the angle_gather dataset.
#         Use "base" for the true/reference model,
#         "high_10" for the +10% (increased) model,
#         "low_10"  for the −10% (decreased) model.
...
font_path = 'C:/Users/owner/Desktop/Curvature/Pretendffsdard-Medium.otf'
try:
    fm.fontManager.addfont(font_path)
    pretendard_font = fm.FontProperties(fname=font_path)
    plt.rcParams['font.family'] = pretendard_font.get_name()
    plt.rcParams['axes.unicode_minus'] = False
    print("Pretendard font registration successful:", pretendard_font.get_name())
except Exception as e:
    print(f"Pretendard font load failed: {e}")
    print("Using default font.")
    plt.rcParams['font.family'] = 'DejaVu Sans'
    plt.rcParams['axes.unicode_minus'] = False


# Global variables to store all curvature data from all angle gathers
all_curvature_data = []
all_position_data = []
all_gather_numbers = []
...
def create_curvature_visualization():
    ...
    for group_idx, (curv_values, pos_values, gather_num) in enumerate(zip(all_curvature_data, all_position_data, all_gather_numbers)):
        ...
        for data_idx, (pos_val, curv_val) in enumerate(zip(pos_values, curv_values)):
            if isinstance(pos_val, (int, float)) and isinstance(curv_val, (int, float)):
                ...
                adjusted_intensity = intensity ** 0.5  
    ...
    # Save figure
    if SAVE_FIGURE:
        heatmap_dir = OUTPUT_DIR         # Put your output directory here
    ...
    output_path = f"{OUTPUT_DIR}/guideline_{folder}.png"
...
for i in range(42):
    ...
    with open(f"{INPUT_DIR}/Data_angle_gather/{folder}/heatmap_{i_gather}.bin") as f:
    ...
    if SAVE_FIGURE:
        out_path = f"{OUTPUT_DIR}/curvature_{i_gather}.png"  
    ...
    print(f"Angle Gather {i_gather:03d} — {len(tot_a)} curvature data saved • {elapsed:,.2f}s")
